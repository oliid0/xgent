...
| 数据域 | Rust 命令/服务 | 表或文件 |
|---|---|---|
| Providers/System/MCP/Agents/Hooks/Cron/Remote/Memory settings | `commands/settings.rs` | `~/.xagent/config.sqlite` 内多张 settings 表 |
| Chat history | `commands/chat_history.rs` | `~/.xagent/chat-history.sqlite3` 的 `chatHistory`、`chatHistorySegment`、`chatHistoryShare`、FTS |
| Memory | `services/memory.rs` | `~/.xagent/memory/**/*.md` + `memory-index.sqlite3` |
| Skills | `services/skills.rs` | `~/.xagent/skills` |
