...
- Mobile Git history now fetches once per panel/workspace opening even when the repository has no commits or a read fails; manual refresh remains available. The previous empty-array effect dependency retriggered indefinitely.
...
- Full non-Cargo suite: 1,277 passed before the later Skill completion guard. Its two relevant Skill suites passed 3 tests afterward; 49 provider request tests passed after the IPv4 proxy correction. pnpm check and lint passed after the Skill guard. Prior native:check and current diff whitespace check passed. The iOS save-status and Rust proxy changes are source-reviewed but cannot be compiled or device-tested on this Windows host under the no-build instruction. Native setup timing and menu hit testing still require device/runtime verification.
