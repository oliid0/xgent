...
export function getToolMeta(name: string): {
  ...
  switch (name) {
    ...
    case "AskUserQuestion":
      return { Icon: MessageCircle, accent: "var(--tool-list-accent)", category: "system" };
    default:
...
export function groupRoundBlocks(blocks: UiRound["blocks"]): GroupedRoundBlock[] {
  ...
  const flushPendingTools = () => {
    ...
    groupedBlocks.push({
      kind: "toolGroup",
      // Keep the Astryx activity wrapper stable from the first ordinary call,
      // so streaming another call updates the same group instead of remounting.
