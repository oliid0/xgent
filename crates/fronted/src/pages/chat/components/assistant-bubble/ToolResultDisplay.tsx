...
export function ToolSection(props: { label?: string; trailing?: ReactNode; children: ReactNode }) {
  ...
  return (
    <AstryxStack direction="vertical" as="section" className="space-y-1.5">
      {label || trailing ? (
        <AstryxStack direction="horizontal" className="flex min-h-5 items-center gap-2">
          {label ? (
            <AstryxText
              as="span"
              type="inherit"
              className="shrink-0 text-[calc(11px*var(--zone-font-scale,1))] font-medium text-muted-foreground/65"
            >
              {label}
            </AstryxText>
          ) : null}
...
export function ToolSurface(props: { children: ReactNode; className?: string }) {
  ...
  return (
    <AstryxStack direction="vertical" className={cn("min-w-0 py-0.5", className)}>
      {children}
    </AstryxStack>
  );
}
...
export function ToolFactGrid({ tags }: { tags: MetaTag[] }) {
  ...
  return (
    <AstryxGrid className="grid gap-x-4 gap-y-1.5 sm:grid-cols-2">
      {tags.map((tag) => (
  ...
  return (
    ...
    >
      <AstryxText
        as="span"
        type="inherit"
        className="min-w-0 flex-1 truncate text-muted-foreground/40"
      >
        {dir.length > 50 ? `…${dir.slice(-50)}` : dir}
      </AstryxText>
      <AstryxText as="span" type="inherit" className="max-w-[70%] truncate text-foreground/85">
        {file}
      </AstryxText>
    </AstryxStack>
  );
...
export function MetaTags({ tags }: { tags: MetaTag[] }) {
  ...
  return (
    <AstryxStack direction="horizontal" className="flex flex-wrap gap-x-3 gap-y-1">
      {tags.map((tag) => {
        ...
        return (
          ...
          >
            <AstryxText as="span" type="inherit" className="font-medium text-muted-foreground/55">
              {tag.label}
            </AstryxText>
            <AstryxText
              as="span"
              type="inherit"
              className="min-w-0 break-all font-mono tabular-nums text-foreground/75"
            >
              {tag.value}
            </AstryxText>
          </AstryxStack>
  ...
  if (rawDetails?.kind === "exit_plan_mode" && typeof rawDetails.plan === "string") {
    ...
    return (
      ...
      >
        ...
        >
          <AstryxText as="span" type="inherit" className="mr-auto text-xs text-muted-foreground">
            {approved
            ...
            {details.entries.map((entry) => (
              ...
              >
                <AstryxText
                  as="span"
                  type="inherit"
                  className="mt-[1px] shrink-0 text-[calc(10px*var(--zone-font-scale,1))] font-semibold uppercase text-muted-foreground/35"
                >
                  {entry.kind}
                </AstryxText>
                <PathDisplay
              ...
              >
                <AstryxStack direction="horizontal" className="flex items-start gap-2">
                  ...
                  <AstryxText
                    as="span"
                    type="inherit"
                    className="shrink-0 rounded bg-black/[0.04] px-1.5 py-[1px] text-[calc(10px*var(--zone-font-scale,1))] font-semibold text-muted-foreground/60 dark:bg-white/[0.05]"
                  >
                    line {match.line}
                  </AstryxText>
                </AstryxStack>
