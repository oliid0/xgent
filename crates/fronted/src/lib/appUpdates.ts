...
export function useAppUpdateController({
  ...
  const setUpdateState = useCallback((next: AppUpdateState) => {
    ...
    if (next.status !== "installed") setNotice(undefined);
    setState(next);
  }, []);
  ...
  const runCheck = useCallback(async () => {
    ...
    if (isAppUpdateBusy(current)) return getAppUpdateStateResult(current);
  ...
  const restart = useCallback(() => {
    if (restartRequestRef.current) return restartRequestRef.current;
    const request = (async () => {
      const current = stateRef.current;
      const result = getAppUpdateStateResult(current);
      if (!result || current.status === "restarting") return;
