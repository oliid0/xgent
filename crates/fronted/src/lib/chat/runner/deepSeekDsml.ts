...
type ParseDsmlToolCallMarkupOptions = { sourceKey?: string };
...
export function parseMarkupAttributes(raw: string) {
  ...
  while (match !== null) {
    ...
    if (key) attributes.set(key, decodeXmlEntities(match[2] ?? ""));
...
function coerceDsmlParameterValue(value: string, attributes: Map<string, string>) {
  ...
  if ((attributes.get("string") ?? "").toLowerCase() === "true") return decoded;
...
export function parseDsmlToolCallMarkup(
  ...
  while (invokeMatch !== null) {
    ...
    if (toolName) {
      ...
      let parameterMatch = DSML_PARAMETER_PATTERN.exec(invokeMatch[2] ?? "");
      while (parameterMatch !== null) {
        const attributes = parseMarkupAttributes(parameterMatch[1] ?? "");
        const name = attributes.get("name")?.trim() ?? "";
        if (name) args[name] = coerceDsmlParameterValue(parameterMatch[2] ?? "", attributes);
        parameterMatch = DSML_PARAMETER_PATTERN.exec(invokeMatch[2] ?? "");
      }
      const signature = [sourceKey, toolCalls.length, toolName, stableStringify(args)].join("\n");
      toolCalls.push({
        type: "toolCall",
        id: `dsml-tool-call-${hashString(signature)}`,
        name: toolName,
        arguments: args,
      });
    }
