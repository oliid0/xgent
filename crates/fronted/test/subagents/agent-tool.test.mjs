...
test("readonly happy path: identity prompts, filtered child tools, SendMessage attached", async () => {
  ...
  assert.equal(call.workdir, "/tmp/xagent-subagent-test");
  assert.match(call.context.systemPrompt, /You are Code Reviewer, a named delegated XAgent subagent/);
...
test("worktree lifecycle: create -> run -> status -> auto apply -> cleanup", async () => {
  ...
  assert.equal(createInput.workdir, "/tmp/xagent-subagent-test");
  assert.match(createInput.label, /^parent-session-call-agent-1-fixer$/);

  const worktreeRoot = "/tmp/xagent-worktrees/parent-session-call-agent-1-fixer";
  ...
  assert.deepEqual(harness.worktreeIpc.applies, [
    { parentWorkdir: "/tmp/xagent-subagent-test", worktreeRoot },
  ]);
  ...
  assert.equal(report.appliedToWorkdir, "/tmp/xagent-subagent-test");
