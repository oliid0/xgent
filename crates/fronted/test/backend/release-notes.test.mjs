...
test("AI release notes script falls back when no API key is configured", () => {
  const dir = mkdtempSync(path.join(tmpdir(), "xagent-notes-"));
  try {
  ...
  const server = http.createServer((request, response) => {
    ...
    request.on("end", () => {
      ...
      response.end(
        JSON.stringify({
          output_text:
            "# XAgent v0.1.6\n\n> Release notes generated by the Responses fallback.\n\n## Overview\n\nXAgent still publishes AI release notes when chat output is empty.",
        }),
      );
    });
  });
