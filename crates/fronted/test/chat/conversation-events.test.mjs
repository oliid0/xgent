...
// Shared conversation events drive native, paired-browser, and mobile views.
import { createTsModuleLoader } from "../helpers/load-ts-module.mjs";

const loader = createTsModuleLoader();
const { createConversationEventController } = loader.loadModule(
  "src/lib/chat/conversation/run/conversationEvents.ts",
);

function createController(options = {}) {
  const sent = [];
  const controller = createConversationEventController({
...
test("conversation event controller emits nothing when disabled", () => {
...
test("conversation token forwarding tracks non-empty text only", () => {
...
test("conversation started control is explicit and does not mark text forwarded", () => {
  const { controller, sent } = createController();
...
test("conversation tool status is normalized and de-duplicated", () => {
  const { controller, sent } = createController();
...
test("conversation retry attempts ride tool_status with the current status and de-duplicate", () => {
  const { controller, sent } = createController();
...
test("conversation close blocks normal events but allows forced title updates", () => {
  const { controller, sent } = createController();
...
test("conversation checkpoint emits compaction summary payload", () => {
  const { controller, sent } = createController();
  const state = {
...
test("conversation user message carries the edit-resend truncation base", () => {
  const { controller, sent } = createController();
...
test("conversation user message omits the truncation base for plain sends", () => {
  const { controller, sent } = createController();
...
test("conversation error can resolve the latest conversation id", () => {
  const { controller, sent } = createController({
