...
enum SnapshotBuilder {
    static func build(
        ...
        observationMode: String = "auto",
        maxImageSize: CGFloat = 1280
    ) throws -> AppSnapshot {
        ...
        let permissions = PermissionDiagnostics.current(requestScreenCapture: observationMode != "text")
        guard permissions.accessibilityTrusted else {
            throw ComputerUseError.permissionDenied("Accessibility permission is required. Grant Xgent access in System Settings > Privacy & Security > Accessibility.")
        }
        ...
        var windowCapture = WindowCapture.resolve(for: app.pid, titleHint: windowTitle, includeImage: observationMode != "text")
        if windowCapture == nil,
           ...
           recoverVisibleWindow(for: app, appElement: appElement, preferredWindow: rootWindow) {
            ...
            if let recoveredWindow = preferredFocusedWindow(appElement: appElement, appPID: app.pid, focusedApplication: focusedApplication, systemWide: systemWide) {
                ...
                windowCapture = WindowCapture.resolve(for: app.pid, titleHint: windowTitle, includeImage: observationMode != "text")
            }
        }
        ...
        return buildAccessibilitySnapshot(
            ...
            includeTree: observationMode != "image",
    ...
    private static func buildAccessibilitySnapshot(
        ...
        let screenshotPNGData = windowCapture.pngDataIfAvailable(maxDimension: maxImageSize)
        ...
        if includeTree { renderer.render(rootElement) }
        if includeTree, let menuBar = copyElement(appElement, attribute: kAXMenuBarAttribute),
...
private struct WindowCapture {
    ...
    func pngDataIfAvailable(maxDimension: CGFloat = 1280) -> Data? {
        guard let image else {
            return nil
        }

        return boundedScreenshotPNGData(for: image, maxDimension: maxDimension)
    }
}
