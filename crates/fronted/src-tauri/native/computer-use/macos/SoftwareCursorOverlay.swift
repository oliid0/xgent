...
func visualCursorEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["XGENT_CUA_VISUAL_CURSOR"]?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() else {
        return true
    }
...
public func visualCursorObservationFileURL(environment: [String: String]) -> URL? {
    guard
        let rawPath = environment["XGENT_CUA_VISUAL_CURSOR_OBSERVATION_FILE"]?
...
public let openComputerUseTurnEndedNotificationName = Notification.Name("com.ohi.xgent.cua.turn-ended")
