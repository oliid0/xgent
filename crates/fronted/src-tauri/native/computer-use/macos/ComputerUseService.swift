...
func validateClickMethod(
    ...
    if method == .global, !globalPointerFallbacksEnabled(environment: environment) {
        throw ComputerUseError.message(
            "click_method 'global' requires XGENT_CUA_ALLOW_GLOBAL_POINTER_FALLBACKS=1 because it may move the system pointer and change foreground focus"
        )
    }
}
...
func inputFallbackDebugEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["XGENT_CUA_DEBUG_INPUT_FALLBACKS"]?
        .trimmingCharacters(in: .whitespacesAndNewlines)
        .lowercased()
    else {
        return false
...
func globalPointerFallbacksEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["XGENT_CUA_ALLOW_GLOBAL_POINTER_FALLBACKS"]?
        .trimmingCharacters(in: .whitespacesAndNewlines)
        .lowercased()
    else {
...
public final class ComputerUseService {
    ...
    private var stateIDs: [pid_t: String] = [:]
    var observationMode = "auto"
    var maxImageSize: CGFloat = 1280

    func cachedState(app query: String) throws -> ToolCallResult {
        let snapshot = try currentSnapshot(for: query)
        guard let stateID = stateIDs[snapshot.app.pid] else {
            throw ComputerUseError.invalidArguments("Observe the app before continuing")
        }
        return .text("state_id: \(stateID)\n" + snapshot.renderedText(style: .fullState))
    }

    func validateActionState(app query: String, stateID: String?) throws -> ToolCallResult? {
        let snapshot = try currentSnapshot(for: query)
        var stale = stateID == nil || stateIDs[snapshot.app.pid] != stateID
        if snapshot.app.pid == ProcessInfo.processInfo.processIdentifier {
            throw ComputerUseError.invalidArguments("Choose a target application other than Xgent")
        }
        if let windowID = snapshot.targetWindowID, let previousBounds = snapshot.windowBounds {
            let windows = CGWindowListCopyWindowInfo(.optionIncludingWindow, windowID) as? [[String: Any]]
            let current = windows?.first { ($0[kCGWindowNumber as String] as? NSNumber)?.uint32Value == windowID }
            let rect = (current?[kCGWindowBounds as String] as? NSDictionary).flatMap { CGRect(dictionaryRepresentation: $0) }
            stale = stale || rect != previousBounds
        }
        if stale {
            let fresh = try getAppState(app: query)
            return ToolCallResult(content: [.text("ACTION NOT APPLIED: missing/stale state_id or changed window. Inspect this fresh state before acting.")] + fresh.content)
        }
        stateIDs.removeValue(forKey: snapshot.app.pid)
        return nil
    }
    ...
    public func scroll(app query: String, direction: String, elementIndex: String?, x: Double? = nil, y: Double? = nil, pages: Double) throws -> ToolCallResult {
        ...
        guard let elementIndex else {
            let point: CGPoint
            if let x, let y {
                point = try screenshotToGlobalPoint(snapshot: snapshot, x: x, y: y)
            } else if let bounds = snapshot.windowBounds {
                point = CGPoint(x: bounds.midX, y: bounds.midY)
            } else {
                throw ComputerUseError.stateUnavailable("Window has no scrollable bounds")
            }
            try performScrollEvent(at: point, direction: normalized, pages: pages, targetDescription: "window", snapshot: snapshot)
            return snapshotResult(for: try refreshSnapshot(for: query), style: .actionResult)
        }
        let record = try lookupElement(snapshot: snapshot, index: elementIndex)
    ...
    private func screenshotToGlobalPoint(snapshot: AppSnapshot, x: Double, y: Double) throws -> CGPoint {
        guard let size = screenshotPixelSize(snapshot: snapshot),
              x >= 0, y >= 0, x < size.width, y < size.height else {
            throw ComputerUseError.invalidArguments("Coordinates require a current screenshot and must lie inside it. Observe with observation=auto/image.")
        }
        return try windowPointToGlobalPoint(
            snapshot: snapshot,
    ...
    private func performNonAXClickFallback(
        ...
        do {
            ...
            guard globalPointerFallbacksEnabled(environment: ProcessInfo.processInfo.environment) else {
                throw ComputerUseError.message(
                    "click could not be handled through accessibility, and global pointer fallback is disabled. Set XGENT_CUA_ALLOW_GLOBAL_POINTER_FALLBACKS=1 to allow physical-pointer fallback for this process."
                )
            }
        }
    }
    ...
    private func snapshotResult(for snapshot: AppSnapshot, style: SnapshotTextStyle) -> ToolCallResult {
        ...
        stateIDs[snapshot.app.pid] = stateID
        var content = [ToolResultContentItem.text("state_id: \(stateID)\n" + snapshot.renderedText(style: style))]
