...
fn ensure_stt_crypto_provider() {
    static INSTALL_DEFAULT_PROVIDER: std::sync::Once = std::sync::Once::new();
...
impl SttManager {
    ...
    async fn start_observed<R: Runtime>(
        ...
        // Desktop STT is independent of remote-control connections, so it must
...
mod tests {
    ...
    #[test]
    fn stt_installs_tls_crypto_without_another_tls_client() {
        ensure_stt_crypto_provider();
