...
mod tests {
    ...
    fn join_url_trims_and_skips_empty_segments() {
        assert_eq!(
            join_url("https://example.com/dav/", &["xagent", "v1"]),
            "https://example.com/dav/xagent/v1"
        );
    ...
    fn jianguoyun_errors_mention_app_password_and_manual_folder() {
        let url = "https://dav.jianguoyun.com/dav/xagent/";
    ...
    /// 凭据只从环境变量读，永不落进仓库：
    /// ```text
    /// XAGENT_WEBDAV_URL=https://dav.jianguoyun.com/dav/ \
    /// XAGENT_WEBDAV_USER=... \
    /// XAGENT_WEBDAV_PASS=... \
    ...
    /// （建目录/写文件/删文件的顺序不确定）。
    #[tokio::test]
    #[ignore = "需要真实 WebDAV 账号，通过 XAGENT_WEBDAV_* 环境变量提供"]
    async fn live_webdav_end_to_end() {
        let (Ok(base_url), Ok(username), Ok(password)) = (
            std::env::var("XAGENT_WEBDAV_URL"),
            std::env::var("XAGENT_WEBDAV_USER"),
            std::env::var("XAGENT_WEBDAV_PASS"),
        ) else {
            eprintln!("跳过：未设置 XAGENT_WEBDAV_URL / _USER / _PASS");
            return;
        };
        ...
        let dir = format!("xagent-livetest-{}", std::process::id());
