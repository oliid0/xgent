...
fn load_message_count_before_segment(
    ...
    .map_err(|e| format!("failed to count messages before edit-resend: {e}"))
}
...
pub(crate) fn chat_history_replace_from_message_sync(
    ...
    let tx = conn
        .transaction()
        .map_err(|e| format!("failed to begin edit-resend transaction: {e}"))?;
    ...
    let location = locate_history_message_ref(std::slice::from_ref(&source_segment), base_message_ref)?;
    ...
    let target_segment = ChatHistorySegmentInput {
        ...
        messages_json: serde_json::to_string(&target_messages)
            .map_err(|e| format!("failed to serialize edit-resend segment: {e}"))?,
    ...
    .map_err(|e| format!("failed to delete edit-resend history suffix: {e}"))?;
    ...
    tx.commit()
        .map_err(|e| format!("failed to commit edit-resend transaction: {e}"))?;
    Ok(result)
}
