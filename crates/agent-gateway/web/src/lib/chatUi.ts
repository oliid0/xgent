...
export function normalizeCheckpointEntry(params: {
  ...
  const coveredMessageCount =
    ...
    coveredMessageCountCandidate > 0
      ...
      : 0;
  const providerId = readString(generatedByRecord.providerId).trim() || "xagent";
...
export function isCheckpointTokenEvent(event: Extract<ChatEvent, { type: "token" }>) {
  return Boolean(
    event.checkpoint ||
      event.api === "xagent-compaction" ||
      (event.provider === "xagent" && event.model === "summary"),
  );
}
...
export function resolveConversationBrowserTitle(params: {
  ...
  const newConversationTitle = params.newConversationTitle.trim() || "XAgent";
